...
export default function AdminPage() {
    ...
    const [modalPin, setModalPin] = useState<Operador | null>(null);
    ...
    const cambiarPin = async () => {
        ...
        try {
            const res = await fetch(`${API}/api/operadores/${modalPin.id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ ...modalPin, pin_acceso: nuevoPin }),
            });
            if (res.ok) {
                alert(`✅ PIN de ${modalPin.nombre} actualizado`);
                setModalPin(null);
                setNuevoPin('');
            } else {
                                    ...
                                    {operadores.map(op => (
                                        <div key={op.id} className="flex items-center justify-between border border-gray-100 rounded-xl px-4 py-3 hover:bg-gray-50 transition">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                <button onClick={() => { setModalPin(op); setNuevoPin(''); }}
                                                    className="p-1.5 rounded-lg bg-purple-50 text-purple-600 hover:bg-purple-100 transition" title="Cambiar PIN">
                                                    <IconKey />
                                                </button>
            ...
            {modalPin && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 px-4">
                    <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm">
                        <h3 className="font-bold text-gray-800 text-lg mb-1">Cambiar PIN</h3>
                        <p className="text-gray-500 text-sm mb-6">{modalPin.nombre} — @{modalPin.usuario}</p>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Nuevo PIN (4-6 dígitos)</label>
                            <input type="password" value={nuevoPin}
                                onChange={e => { if (e.target.value.length <= 6) setNuevoPin(e.target.value); }}
                                maxLength={6}
                                placeholder="••••••"
                        ...
                        <div className="flex gap-3 mt-6">
                            <button onClick={cambiarPin} disabled={guardandoPin}
                                className="flex-1 bg-purple-700 text-white font-bold py-2.5 rounded-xl hover:bg-purple-800 transition disabled:opacity-50">
                                {guardandoPin ? 'Guardando...' : 'Cambiar PIN'}
                            </button>
                            <button onClick={() => setModalPin(null)}
